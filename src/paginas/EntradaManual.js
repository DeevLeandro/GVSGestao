...
export default function EntradaManual() {
  ...
  const calcPreco = (q, c) => (!q || q <= 0 || !c || c <= 0) ? null : c / (q / 1000);
  const calcCusto = (q, p) => (!q || q <= 0 || !p || p <= 0) ? null : (q / 1000) * p;
  ...
  const handleQuantidadeChange = (value) => {
    ...
    if (custoTotal > 0 && editando === "custo") {
      const p = calcPreco(q, custoTotal);
      setPrecoMilheiro(p);
      form.setFieldValue("precoMilheiro", formatLocaleNumber(p));
    } else if (precoMilheiro > 0 && editando === "preco") {
      const c = calcCusto(q, precoMilheiro);
      setCustoTotal(c);
      form.setFieldValue("custoTotal", formatLocaleNumber(c));
    }
  };
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      await api.post("/ServerPrincipal/InserirEntradas", {
        IDCliente: clienteId,
      ...
      message.success(`Entrada realizada! +${qtd.toLocaleString("pt-BR")} milhas`);
      
      form.resetFields(["quantidade", "custoTotal", "precoMilheiro", "observacoes"]);
      setQtd(null);
      setPrecoMilheiro(null);
      setCustoTotal(null);
      setEditando(null);
      ...
      message.error(error.response?.data || "Erro ao realizar entrada manual");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Spin spinning={loading}>
        <div style={cardStyle}>
          <div style={headerStyle}>
            <EditOutlined style={{ fontSize: 17, color: "#555" }} />
