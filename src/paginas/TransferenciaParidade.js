...
export default function TransferenciaComParidade({ clienteId: clienteIdProp }) {
  ...
  const [paridade, setParidade] = useState(1.0);
  ...
  const pontosOrigem  = cartaoOrig?.car_saldo_milhas || 0;
  const custoMilheiro = cartaoOrig?.car_vunitario ?? cartaoOrig?.CAR_VUNITARIO ?? 0;
  const pontosDestino = Math.round(pontosOrigem * paridade);
  const custoTotal    = custoMilheiro > 0 ? (pontosOrigem / 1000) * custoMilheiro : 0;
  ...
  const handleOrigemChange = (id) => {
    ...
    if (form.getFieldValue("cartaoDestinoId") === id)
      form.setFieldValue("cartaoDestinoId", undefined);
  };

  const destinosDisponiveis = (origemId) =>
    origemId ? cartoes.filter((c) => c.car_id !== origemId) : cartoes;

  // ── Submit ─────────────────────────────────────────────────────────────
  const handleSubmit = async (values) => {
    if (!clienteId) { message.error("Cliente não identificado."); return; }
    if (values.cartaoOrigemId === values.cartaoDestinoId) {
      message.error("Origem e destino devem ser diferentes."); return;
    }
    setLoading(true);
    try {
      await api.post("/ServerPrincipal/InserirTransferenciaParidade", {
        IDCliente:     clienteId,
      ...
      message.success(`Transferência realizada! ${pontosDestino.toLocaleString("pt-BR")} pontos transferidos`);
      form.resetFields();
      setParidade(1.0); setCartaoOrig(null);
    } catch (e) {
  ...
  const isFormValid = !!origemSel && !!destinoSel;

  // ── Helper Option com logo ─────────────────────────────────────────────
  const CartaoOption = ({ c }) => {
  ...
  const lbl = { fontSize: 13, color: "#666", fontWeight: 500, marginBottom: 4, display: "block" };
  const inp = { width: "100%", borderRadius: 4, border: "1px solid #d9d9d9", background: "#fafafa", height: 40, fontSize: 14 };
  const dis = { ...inp, background: "#f5f5f5", color: "#999", cursor: "not-allowed" };
  const fi  = { marginBottom: 14 };

  return (
    <div style={{ minHeight: "100vh", background: "#f0f2f5", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "32px 24px" }}>
      <Spin spinning={loading}>
        <div style={{ width: "100%", maxWidth: 1080, borderRadius: 8, boxShadow: "0 1px 4px rgba(0,0,0,0.1)", background: "#fff" }}>

          {/* Header */}
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 32px 16px", borderBottom: "1px solid #f0f0f0" }}>
            <SwapOutlined style={{ fontSize: 18, color: "#555" }} />
            <span style={{ fontSize: 16, fontWeight: 600, color: "#333" }}>Transferência com Paridade</span>
          </div>

          <div style={{ padding: "24px 32px 32px" }}>
            <Form form={form} layout="vertical" onFinish={handleSubmit}
              initialValues={{ dataOperacao: dayjs(), paridade: "1,0", validade: 24 }}
            >
                  ...
                  <div style={fi}>
                    ...
                    <Form.Item name="cartaoOrigemId" noStyle rules={[{ required: true }]}>
                      <Select size="middle" style={{ width: "100%" }}
                        placeholder="Selecione a origem" loading={loading}
                ...
                <Col xs={24} sm={11}>
                  <div style={fi}>
                    ...
                    <Form.Item name="cartaoDestinoId" noStyle rules={[{ required: true }]}>
                      ...
                      >
                        {destinosDisponiveis(origemSel).map((c) => (
                          <Option key={c.car_id} value={c.car_id}>
                            <CartaoOption c={c} />
                          </Option>
                        ))}
              ...
              <Row gutter={16} style={{ marginBottom: 14 }}>
                ...
                <Col xs={24} sm={12}>
                  <div style={fi}>
                    ...
                    <Input size="middle" style={dis} readOnly
                      value={custoMilheiro > 0 ? formatLocaleNumber(custoMilheiro) : ""}
              ...
              <Row gutter={24}>
                ...
                <Col xs={24} md={12} style={{ borderRight: "1px dashed #e0e0e0" }}>
                  <div style={fi}>
                    <label style={lbl}>Pontos Origem</label>
                    <Input size="middle" style={dis} readOnly
                      value={cartaoOrig ? (cartaoOrig.car_saldo_milhas || 0).toLocaleString("pt-BR") : ""}
                      suffix={<span style={{ color: "#bbb" }}>#</span>}
                    />
                  ...
                  <div style={fi}>
                    <label style={lbl}>Paridade</label>
                    <Row gutter={8} align="middle">
                      <Col flex="1">
                        <InputNumber size="middle" style={inp} min={0.01} step={0.1} precision={1}
                          controls={false} value={paridade}
                          onChange={(v) => setParidade(Number(v) || 1)}
                          formatter={(v) => v !== undefined && v !== null ? String(v).replace(".", ",") : ""}
                          parser={(v) => v ? parseFloat(v.replace(",", ".")) : 1}
                        />
                      </Col>
                      <Col style={{ color: "#888", fontSize: 13, whiteSpace: "nowrap" }}>: 1</Col>
                    </Row>
                  </div>
                  ...
                  <div style={fi}>
                    ...
                    <Form.Item name="observacoes" noStyle>
                      <Input.TextArea rows={5} style={{ width: "100%", borderRadius: 4, border: "1px solid #d9d9d9", fontSize: 13, resize: "vertical" }} />
                    </Form.Item>
                  </div>
                </Col>
                ...
                <Col xs={24} md={12}>
                  <div style={fi}>
                    <label style={lbl}>Custo do Milheiro</label>
                    <Input size="middle" style={dis} readOnly
                      value={custoMilheiro > 0 ? formatLocaleNumber(custoMilheiro) : ""}
                      placeholder="0,00" suffix={<span style={{ color: "#bbb" }}>$</span>}
                    />
                  ...
                  <div style={fi}>
                    <label style={lbl}>Pontos Destino</label>
                    <InputNumber size="middle" style={dis} disabled value={pontosDestino || null}
                      controls={false}
                  ...
                  <div style={fi}>
                    ...
                    <Form.Item name="validade" noStyle>
                      <Select size="middle" style={{ width: "100%" }}>
